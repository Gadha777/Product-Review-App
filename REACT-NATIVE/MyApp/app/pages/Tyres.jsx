...
import productImage from "../../assets/images/tyre2.jpeg";
...
const Tyres = () => {
  ...
  useEffect(() => {
    const fetchReviews = async () => {
      try {
        const res = await fetch("http://localhost:5000/reviews/category/Tyres");
    ...
    <View style={styles.container}>
      ...
      <ScrollView>
        {reviews && Object.keys(reviews).length ? (//Ensures that there is at least one review.
          ...
          <Text style={styles.noReviews}>No reviews available for Tyres.</Text>
        )}
      </ScrollView>
    </View>
  );
