...
import productImage from "../../assets/images/image copy 2.png";
...
const Vacuum = () => {
  ...
  useEffect(() => {
    const fetchReviews = async () => {
      try {
        const res = await fetch("http://localhost:5000/reviews/category/Vacuum");
    ...
    <View style={styles.container}>
      ...
      <ScrollView>
        {reviews && Object.keys(reviews).length ? (//Ensures that there is at least one review.
          ...
          <Text style={styles.noReviews}>No reviews available for Vacuums.</Text>
        )}
      </ScrollView>
    </View>
  );
