...
import productImage from "../../assets/images/Home.jpg";
...
const Homebuilders = () => {
  ...
  useEffect(() => {
    const fetchReviews = async () => {
      try {
        const res = await fetch("http://localhost:5000/reviews/category/Home");
    ...
    <View style={styles.container}>
      ...
      <ScrollView>
        {reviews && Object.keys(reviews).length ? (//Ensures that there is at least one review.
          ...
          <Text style={styles.noReviews}>No reviews available for Homebuilders.</Text>
        )}
      </ScrollView>
    </View>
  );
